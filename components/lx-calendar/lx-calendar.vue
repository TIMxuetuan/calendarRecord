<template>
	<view class="date" style="height:100%">
		<!-- <view class="head">
		...
		<swiper style="height:100%" :current="current" circular @change="change_date">
			<swiper-item>
				<view class="daysItem">
					<view class="date_dl" v-show="!retract || index == to_prev_week_index" v-for="(item,index) in week_list_prev_co"
					 :key="index">
			...
			<swiper-item>
				<view class="daysItem">
					<view class="date_dl" v-show="!retract || index == to_next_week_index" v-for="(item,index) in week_list_next_co"
					 :key="index">
						<view class="dd" @click="item_click(vo,index,key)" v-for="(vo,key) in item" :key="key">
							<view class="num" :class="[vo.today ? 'today' : '',vo.type == 'month' ? 'month' : (retract ? '' : 'disabled')]">{{vo.day}}</view>
							<view v-show="vo.dot && (vo.type == 'month' || retract)" class="dot"></view>
			...
			doc_list_update(week_list = 'week_list') {
				...
				this[week_list].map((item, index) => {
					list.push(item.map((vo, key) => {
						if (this.dot_list.indexOf(vo.date) > -1 || this.dot_list.indexOf(vo.date.replace(/-(\d)(?!\d)/g, '-0$1')) >
							-1) {
							vo.dot = true;
