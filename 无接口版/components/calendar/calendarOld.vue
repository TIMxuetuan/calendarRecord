	...
	export default {
		...
		methods: {
			...
			swiperChange(e) {
				...
				console.log("isLoopPositive",isLoopPositive)
				if (current - currentIndex == 1 || isLoopPositive) {
					this.nextYearMonth = this.nextMonth(this.nextYearMonth.year, this.nextYearMonth.month) // 获取下月 年、月
					console.log("this.nextYearMonth",this.nextYearMonth)
					this.getPreLast(this.nextYearMonth.year, this.nextYearMonth.month);
					// this.getPreLast(this.preYearMonth.year, this.preYearMonth.month);
				}
				// 反向滑动，到上一个的时候
